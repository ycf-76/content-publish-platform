...
> ⚠️ **使用前请先完成配置：**
> - 默认品牌色可直接使用，如需替换成你自己的请同步修改模板变量
> - **必须放入你自己的头像**，不得使用原作者的头像/IP形象
> - **必须替换为你自己的姓名、IP和品牌信息**
>
> 本系统开源的是设计方法论、设计规范、工作流程、布局模式、组件模式和相关模板。
> 你可以基于 CC BY-NC-SA 4.0 学习、修改和分享这些内容，但这不代表你获得了使用
> **ESTHER不二 / Esther / 不二 / esthersjw** 的姓名、头像、IP形象、Logo、品牌标识、
> 个人账号标识或本人形象进行创作、运营、发布、商业合作或对外背书的许可。
>
> 任何使用本系统内容制作的账号、作品、产品、课程、Agent 或服务，都不得让人误以为
> 由原作者制作、授权、合作或背书。协议要求的署名仅表示内容来源，不等于身份授权。
...
> ⚠️ **不得使用原作者的头像或IP形象。** 请替换为你自己的头像和品牌标识。

请将**你自己的**头像文件放入 `assets/avatar.jpg`（建议正方形，至少 400×400px）。

如果你有完整IP形象（全身/半身），放入 `assets/character.png`。

### 使用规则
- 需要头像时优先用 `assets/avatar.jpg`（你自己的头像）
- 是否在页面中使用IP形象由你决定，不强制
- **严禁使用 Esther/不二 的头像、IP形象或任何可识别为原作者的视觉元素**
...
- **温暖但不幼稚** — 有内容有深度
- **个人品牌感** — 一看就知道是"你的"，而不是别人的

> 💡 请根据你自己的品牌调性修改上面的关键词。这是方法论，不是身份——输出的作品应体现使用者的品牌，而非原作者。
